...
function displayCommunities(data) {
    let communityList = $("#community-browse")
    var htmlString = ""

    data.map((community) => {
        htmlString += `
            <div class="text-lg bg-gray-200 border-4 border-gray-200 rounded-xl">
                <div class="h-48 bg-gray-200">`
                
        if (community.has_cover_img) {
            htmlString += `<img class="object-cover rounded-t-lg h-full w-full" src="https://stolploftqaslfirbfsf.supabase.in/storage/v1/object/public/public/communities/cover_img/${community.id}.jpg"/>`
        }

        htmlString += `</div><div class="rounded-full h-32 w-32 bg-gray-400 mx-auto -mt-40 mb-8">`
